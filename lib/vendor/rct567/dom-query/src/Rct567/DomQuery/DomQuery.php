...
class DomQuery extends DomQueryNodes
{
    ...
    public function html($html_string=null)
    {
        ...
        if ($html_string !== null) { // set html for all nodes
            $first = substr(trim($html_string),0,1);
            $last = substr(trim($html_string),-1);
            if (!in_array($first,["<",">"]) OR !in_array($last,["<",">"])) {
              if (!in_array($first,["<",">"])) {
                  // Fix problem with <p> tag in mixed text + tag node
                  $pos = strpos($html_string,"<");
                  $text = substr($html_string,0,$pos);
                  $html_string = substr($html_string,$pos);
                  ...
                  // Fix problem with <p> tag in mixed tag + text node
                  $arr = explode(">",$html_string);
                  $text = array_pop($arr);
                  $html_string = implode(">",$arr);
                  foreach ($this as $node) {
                      $node->get(0)->nodeValue = '';
                      $node->append($html_string);
                      $node->append($text);
    ...
    public function after()
    {
        $this->importNodes(\func_get_args(), function ($node, $imported_node) {
            if ($node->nextSibling) {
                ...
                if ($node->parentNode) $node->parentNode->appendChild($imported_node);
            }
        });
