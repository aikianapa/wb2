<?php
    function tagThumbnail(&$dom) {
        ...
        if ($src=="") $src="0";
        $noimg=$dom->attr("data-wb-noimg");
        $form=$dom->attr("data-wb-form");
        if ($form=="" && isset($Item["form"])) $form=$Item["form"];
        $from=$dom->attr("data-wb-from");
        if ($from=="") $from="images";
        $item=$dom->attr("data-wb-item");
        if ($item=="" && isset($Item["id"])) $item=$Item["id"];
        $show=$dom->attr("data-wb-show");
        ...
        $offset=$dom->attr("offset");
        $contain=$dom->attr("contain");
